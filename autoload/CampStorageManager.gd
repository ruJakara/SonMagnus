...
func _ready():
	var upgrade_manager := get_node_or_null("/root/CampUpgradeManager")
	if upgrade_manager:
...
func unreserve(reservation_id: String, items: Dictionary = {}) -> Dictionary:
	...
	if items.is_empty():
		...
		for item_id in items.keys():
			var amount := clamp(int(items[item_id]), 0, stored.get(item_id, 0))
			if amount == 0:
				continue
	...
	for item_id in released.keys():
		var amount := int(released[item_id])
...
func consume_reserved(reservation_id: String, items: Dictionary = {}) -> bool:
	if not _reservations.has(reservation_id):
		return false
	var stored: Dictionary = _reservations[reservation_id]
	var payload := items if not items.is_empty() else stored
	for item_id in payload.keys():
		var amount := int(payload[item_id])
