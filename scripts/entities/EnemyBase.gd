...
## Базовый класс врага: обёртка над BaseEntity + AI/атака/анимации.
##
## Обновлено для использования Area2D-зон:
## - Добавлены ссылки на hurtbox и backbox
## - Настройка слоев столкновений для корректной работы зон
class_name EnemyBase
extends BaseEntity

signal died_with_id(enemy_id: String)

@export var enemy_id: String = "unknown_enemy"
@export var enemy_name: String = "Безымянный враг"
@export var crit_chance: float = 0.05
@export var data_path: String = ""  # JSON врага
...
@onready var backbox: Area2D = $Back

# Настройка слоев столкновений:
# Hurtbox должен ловить атаки игрока (layer 4 - attack hitbox)
# Back должен ловить атаки игрока со спины (тот же слой, но другая логика обработки)

func _ready() -> void:
	# базовая инициализация из BaseEntity
...
func take_damage(amount: int) -> void:
	...
	_play_hit_reaction(get_tree().get_first_node_in_group("player"))
...
func _play_hit_reaction(from: Node) -> void:
	...
	if body and from and from is Node2D:
		var attacker := from as Node2D
		var dir: Vector2 = (body.global_position - attacker.global_position).normalized()
		body.velocity += dir * 40.0
...
func die() -> void:
	state = "dead"
	emit_signal("died_with_id", enemy_id)
	emit_signal("died")  # сигнал из BaseEntity
