...
func get_animatedsprite():
	...
	if sprites.is_empty() or drop_down.selected >= sprites.size():
		return null
...
func get_items():
	...
	if anim_sprites.is_empty():
		drop_down.add_item("No AnimatedSprite2D/3D found", 0)
		drop_down.disabled = true
		return
...
func convert_sprites():
	...
	if not animated_sprite:
		printerr("[AS2P] No AnimatedSprite2D/3D selected!")
		return
	...
	if not sprite_frames:
		printerr("[AS2P] Selected AnimatedSprite2D has no frames!")
		return
...
func add_animation(anim_sprite: NodePath, anim: String, sprite_frames: SpriteFrames):
	...
	# ИСПРАВЛЕНИЕ: удаляем в обратном порядке (индексы не сдвигаются)
	var tracks_to_remove = []
	if anim_track >= 0:
		tracks_to_remove.append(anim_track)
	if frame_track >= 0:
		tracks_to_remove.append(frame_track)
	
	# Сортируем по убыванию и удаляем
	...
	# ИСПРАВЛЕНИЕ: не дублируем add_animation (уже добавили выше)
	# global_animation_library.add_animation(sanitized_anim_name, animation)
